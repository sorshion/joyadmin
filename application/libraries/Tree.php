...
class Tree {
	...
	*/
	...
	public function get_array($parent_id=0)
	{
		$arr = array();
		$rows = $this->get_child($parent_id);

		if($rows)
			foreach($rows as $k=>$v)
			{
				$v['child'] =  $this->get_array($v['menu_id']);
	...
	* @return string
	*/
	public function get_tree($myid, $str, $sid = 0, $adds = '', $str_group = '',$depth=0){
		$number=1;
		...
		if(is_array($child)){
			...
			foreach($child as $id=>$a){
				...
				$this->get_tree($id, $str, $sid, $adds.$k.$nbsp,$str_group,$depth);
